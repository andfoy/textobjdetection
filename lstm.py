...
train_corpus = corpus.tokenize_file(osp.join(args.data, 'train.txt'))
val_corpus = corpus.tokenize_file(osp.join(args.data, 'val.txt'))
test_corpus = corpus.tokenize_file(osp.join(args.data, 'test.txt'))
...
val_data = batchify(val_corpus, eval_batch_size)
test_data = batchify(test_corpus, eval_batch_size)
...
def train():
    ...
    ntokens = len(corpus.dictionary)
    hidden = model.model.init_hidden(args.batch_size)
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        data, targets = get_batch(train_data, i)
        """
        Starting each batch, we detach the hidden state from
        how it was previously produced.
...
try:
    for epoch in range(1, args.epochs + 1):
        ...
        if not best_val_loss or val_loss < best_val_loss:
            ...
            has been seen in the validation dataset.
            """
            lr /= 4
